...
std::vector<std::vector<Image::SubresourceLayoutState>> CreateSubresourceStates(const ImageInfo& info) {
	return std::vector<std::vector<Image::SubresourceLayoutState>>(
		info.mLayers,
		std::vector<Image::SubresourceLayoutState>(
			...
			Image::SubresourceLayoutState{
				...
				info.mQueueFamilies.empty() ? VK_QUEUE_FAMILY_IGNORED : info.mQueueFamilies.front() }));
}
...
Image::Image(Device& device, const ImageInfo& info, const vk::MemoryPropertyFlags memoryFlags, const VmaAllocationCreateFlags allocationFlags) : mInfo(info) {
	...
	vk::ImageCreateInfo createInfo(
		...
		mInfo.mUsage,
		mInfo.mSharingMode,
		mInfo.mQueueFamilies,
		vk::ImageLayout::eUndefined );

	vk::Result result = (vk::Result)vmaCreateImage(device.mMemoryAllocator, &(const VkImageCreateInfo&)createInfo, &allocationCreateInfo, &(VkImage&)mImage, &mAllocation, nullptr);
...
const vk::ImageView Image::GetView(const Device& device, const vk::ImageSubresourceRange& subresource, const vk::ImageViewType viewType, const vk::ComponentMapping& componentMapping) {
